...
❗ 외부함수(useState)가 반환한 내부함수(setState)는 외부함수(useState)의 호출이 끝났음에도 자신이 선언된 외부 함수가 선언된 환경을 깅거해 계속 state값을 사용할 수 있다.
...
위 코드의 경우 i가 var로 선언되어있다. var는 for문과 상관없이 해당 구문이 선언된 함수 렙레 스코프를 바라보므로 함수 내부 실행이 아니라면 전역 스코프에 등록 되어있을 것이다.
